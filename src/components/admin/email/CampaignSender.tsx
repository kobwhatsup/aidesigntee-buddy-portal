...
export function CampaignSender({ campaign, onCampaignUpdate }: CampaignSenderProps) {
  ...
  const handleSendCampaign = async () => {
    ...
    try {
      ...
      toast({
        title: "发送启动成功",
        description: `已开始向 ${data.recipients} 位用户发送邮件`,
      });
  ...
  const isSendingOrSent = ['sending', 'sent', 'partial'].includes(campaign.status);
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <Badge variant={campaign.status === 'draft' ? 'secondary' : 'default'} className="ml-2">
                ...
                {campaign.status === 'sent' && '已发送'}
                {campaign.status === 'partial' && '部分成功'}
                {campaign.status === 'failed' && '发送失败'}
              </Badge>
            </div>
          ...
          {isSendingOrSent && (
            <div className="pt-4 border-t">
              <p className="text-sm text-gray-600 mb-2">
                {campaign.status === 'sending' && '邮件正在发送中，请稍候...'}
                {campaign.status === 'sent' && '邮件发送已完成'}
                {campaign.status === 'partial' && '邮件发送部分完成'}
              </p>
            </div>
          )}
        </CardContent>
